(() => {
  ...
  function cacheElements() {
    Object.assign(els, {
      pages: qa(".page"),
      navItems: qa("[data-page]"),
      pageTitle: q("#pageTitle"),
      pageSubtitle: q("#pageSubtitle"),
      pageEyebrow: q("#pageEyebrow"),
      headerName: q("#headerName"),
      profileName: q("#profileName"),
      avatarLetter: q("#avatarLetter"),
      sidebarStreak: q("#sidebarStreak"),
      todayTimeHero: q("#todayTimeHero"),
      goalRemaining: q("#goalRemaining"),
      heartPercent: q("#heartPercent"),
      mainHeart: q("#mainHeart"),
      todayTime: q("#todayTime"),
      dailyGoalLabel: q("#dailyGoalLabel"),
      progressRing: q("#progressRing"),
      progressRingText: q("#progressRingText"),
      dailyProgressBar: q("#dailyProgressBar"),
      subjectList: q("#subjectList"),
      streakDays: q("#streakDays"),
      weekDays: q("#weekDays"),
      streakMessage: q("#streakMessage"),
      focusSubject: q("#focusSubject"),
      durationSelect: q("#durationSelect"),
      timerDisplay: q("#timerDisplay"),
      timerCaption: q("#timerCaption"),
      timerHeart: q("#timerHeart"),
      timerRing: q("#timerRing"),
      focusStatus: q("#focusStatus"),
      startPauseTimer: q("#startPauseTimer"),
      resetTimer: q("#resetTimer"),
      finishTimer: q("#finishTimer"),
      totalStudyTime: q("#totalStudyTime"),
      averageStudyTime: q("#averageStudyTime"),
      statsStreak: q("#statsStreak"),
      sessionCount: q("#sessionCount"),
      weeklyChart: q("#weeklyChart"),
      heartCollection: q("#heartCollection"),
      collectionText: q("#collectionText"),
      nextHeartText: q("#nextHeartText"),
      historyList: q("#historyList"),
      achievementGrid: q("#achievementGrid"),
      settingsModal: q("#settingsModal"),
      subjectModal: q("#subjectModal"),
      settingsForm: q("#settingsForm"),
      subjectForm: q("#subjectForm"),
      nameInput: q("#nameInput"),
      goalInput: q("#goalInput"),
      subjectInput: q("#subjectInput"),
      themeButton: q("#themeButton"),
      toast: q("#toast")
    });
  }
  ...
  function bindNavigation() {
    qa("[data-page]").forEach(button => {
      button.addEventListener("click", () => navigate(button.dataset.page));
    });
    qa("[data-page-jump]").forEach(button => {
      button.addEventListener("click", () => navigate(button.dataset.pageJump));
    });
    qa("[data-go-focus]").forEach(button => {
      button.addEventListener("click", () => navigate("foco"));
    });
  }
  ...
  function finishSession(completed) {
    ...
    saveData();
    renderAll();
    showToast(completed ? "Sessão concluída! Seu coração ficou mais forte. 💗" : `Sessão de ${elapsedMinutes} min salva com sucesso.`);
    resetTimer();
  ...
  function bindModals() {
    ...
    [els.settingsModal, els.subjectModal].forEach(modal => {
    ...
    els.settingsForm.addEventListener("submit", event => {
      ...
      const name = els.nameInput.value.trim() || "Estudante";
    ...
    els.subjectForm.addEventListener("submit", event => {
      ...
      if (!subject) return;
      if (state.data.subjects.some(item => item.toLowerCase() === subject.toLowerCase())) {
        showToast("Essa matéria já existe.");
        return;
      }
  ...
  function updateThemeButton() {
    els.themeButton.textContent = state.data.profile.dark ? "☀" : "☾";
  }
  ...
  function renderProfile() {
    ...
    els.headerName.textContent = firstName(name).toLowerCase();
  }
  ...
  function renderDashboard() {
    ...
    els.streakMessage.textContent = streak
      ? `Você está há ${streak} ${streak === 1 ? "dia" : "dias"} cuidando da sua rotina.`
      ...
      return `
        <div class="subject-item">
          <div class="subject-item-top">
            <span class="subject-badge">${symbols[index % symbols.length]}</span>
            <span class="status-dot">${state.data.sessions.filter(s => s.subject === subject).length} sessões</span>
          </div>
          <strong>${escapeHtml(subject)}</strong>
          <small>${formatMinutes(total)} estudados</small>
          <div class="subject-mini-bar"><span style="width:${pct}%"></span></div>
        </div>
      `;
    }).join("");
  }
  ...
  function renderFocusSubjects() {
    ...
    els.focusSubject.innerHTML = state.data.subjects
  ...
  function renderWeekDays() {
    ...
    els.weekDays.innerHTML = days.map(date => {
      ...
      return `
        <div class="day-bubble ${completed ? "completed" : ""} ${key === todayKey ? "today" : ""}">
          <span>${completed ? "✓" : date.getDate()}</span>
          <small>${labels[date.getDay()]}</small>
        </div>
      `;
    }).join("");
  }
  ...
  function renderStats() {
    ...
    const weekData = lastSevenDays().map(date => ({
      date,
      minutes: minutesOnDate(date)
    }));
    ...
    els.weeklyChart.innerHTML = weekData.map(item => {
      ...
      return `
        <div class="bar-column">
          <span class="bar-value">${item.minutes ? formatMinutes(item.minutes) : "0"}</span>
          <div class="bar" style="height:${height}%"></div>
          <span class="bar-day">${dayNames[item.date.getDay()]}</span>
        </div>
      `;
    }).join("");

    const shown = Math.min(20, state.data.sessions.length);
    els.heartCollection.innerHTML = Array.from({ length: 20 }, (_, index) =>
      `<span class="collection-heart ${index < shown ? "" : "empty"}">♥</span>`
    ).join("");
    ...
    const nextMilestone = Math.ceil((state.data.sessions.length + 1) / 5) * 5;
    els.nextHeartText.textContent = `Próximo marco: ${nextMilestone}`;
    ...
    els.historyList.innerHTML = state.data.sessions.slice(0, 8).map(session => {
      ...
      return `
        <div class="history-item">
          ...
          <span class="history-time">${formatMinutes(session.minutes)}</span>
  ...
  function renderAchievements() {
    ...
    els.achievementGrid.innerHTML = achievements.map(item => `
      <article class="achievement-card ${item.unlocked ? "" : "locked"}">
        <div class="achievement-badge">${item.unlocked ? item.icon : "🔒"}</div>
        <h3>${item.title}</h3>
        <p>${item.description}</p>
        <small>${item.unlocked ? "Desbloqueada" : item.progress}</small>
      </article>
    `).join("");
  }
  ...
  function studiedSubjectCount() {
    return new Set(state.data.sessions.map(s => s.subject)).size;
  }
  ...
  function firstName(name) {
    return String(name || "Estudante").trim().split(/\s+/)[0] || "Estudante";
  }
  ...
  function showToast(message) {
    ...
    showToast.timeout = setTimeout(() => els.toast.classList.remove("show"), 3200);
  }
